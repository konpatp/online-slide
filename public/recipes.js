...
"use strict";
(() => {
  // src/recipes/guides.js
  var scientificRecipeGuides = {
  ...
  // src/recipes/heroPlot.js
  function createHeroPlot(api) {
    const global = window;
    ...
    function slideIndex(canvas, slide) {
      ...
      function visibilityButton(ids) {
        ...
        button.addEventListener("click", function(event) {
          ...
          api.setSlidesHidden(ids, !hidden);
        });
        return button;
      }
      slide.data.sections.forEach(function(section) {
        ...
        head.appendChild(visibilityButton(section.items.map(function(item) {
          return item.slide;
        })));
        group.appendChild(head);
        var links = document.createElement("div");
        links.className = "index-links";
        section.items.slice().sort(function(a, b) {
          return api.orderOfSlide(a.slide) - api.orderOfSlide(b.slide);
        }).forEach(function(item) {
          var row = document.createElement("div");
          row.className = "index-link-row";
          ...
          link.addEventListener("click", function(event) {
            if (api.isEditMode()) event.preventDefault();
          });
          row.appendChild(link);
          row.appendChild(visibilityButton([item.slide]));
  ...
  function createEvidenceTable(api) {
    ...
    function evidenceTable(canvas, slide) {
      if (slide.data.tables) {
        ...
        if (selector) {
          try {
            active = localStorage.getItem(storageKey) || active;
          } catch (_) {
          }
      ...
      var totalWidth = model.columns.reduce(function(sum, column) {
  ...
  // src/recipes/targetAccessibility.js
  function createTargetAccessibility(api) {
    const global = window;
    ...
    function targetAccessibility(canvas, slide) {
      ...
      slide.data.panels.forEach(function(panel) {
        var total = panel.shares.reduce(function(sum, value) {
          return sum + value;
        }, 0);
        ...
        records.push({
          id: panel.id + "-target",
          kind: "accessibility-target",
          mode: "rect",
          article,
          element: bar
        });
        records.push({
          id: panel.id + "-b4-reach",
          kind: "accessibility-reach",
          mode: "line",
          article,
          element: b4Line
        });
        records.push({
          id: panel.id + "-r3-reach",
          kind: "accessibility-reach",
          mode: "line",
          article,
          element: r3Line
    ...
    function wireVisualObjects(slide, records) {
      ...
      function lineGeometry(record) {
        ...
        return {
          kind: record.kind,
          from: [
            rounded((box.left - outer.left) / outer.width),
            rounded((box.top + box.height / 2 - outer.top) / outer.height)
          ],
          to: [
            rounded((box.right - outer.left) / outer.width),
            rounded((box.top + box.height / 2 - outer.top) / outer.height)
          ]
        };
      }
      ...
      function startGesture(record, gesture, event) {
        ...
        function onEnd() {
          ...
          if (moved) api.updateVisualObject(
            slide.id,
            record.id,
            record.kind,
            objectState[record.id],
            true
          );
        }
      ...
      records.forEach(function(record) {
        ...
        if (record.mode === "rect") {
          ...
          if (record.kind === "recipe-frame") {
            ...
            selector.className = "recipe-frame-select";
          ...
          [
            ["start", "accessibility-line-endpoint"],
            ["end", "accessibility-line-endpoint"],
            ["move", "accessibility-line-move"]
  ...
  // src/recipes/annotations.js
  function createAnnotations(api) {
    const global = window;
    ...
    function annotations(canvas, slide) {
      ...
      (slide.annotations || []).forEach(function(item) {
        ...
        records.push({
          id: item.id,
          kind: item.kind === "rect" ? "annotation-rect" : "annotation-line",
          mode: item.kind === "rect" ? "rect" : "line",
          article: canvas,
          element,
  ...
  // src/recipes/mechanismPipeline.js
  function createMechanismPipeline(api) {
    const global = window;
    ...
    function mechanismPipeline(canvas, slide) {
      ...
      if (api.isEditMode()) paperHost.addEventListener("click", function(event) {
        event.stopPropagation();
      });
      ...
      function measureNodes() {
        ...
        slide.data.nodes.forEach(function(node) {
          ...
          var minWidth = Math.max(154, Math.min(220, planeWidth * 0.13));
          var detail = node.detail && slide.components[node.detail];
          var mathDetail = detail && detail.render === "latex";
          var maxWidth = Math.max(minWidth, Math.min(mathDetail ? 400 : 360, planeWidth * 0.32));
  ...
  // src/recipes/vectorGeometry.js
  function createVectorGeometry(api) {
    const global = window;
    ...
    function vectorGeometry(canvas, slide) {
      ...
      (slide.data.labels || []).forEach(function(label) {
        ...
        if (label.space === "world") worldLabels.push({ label, region });
      ...
      requestAnimationFrame(function() {
        ...
        var geometry = window.ScientificGeometryRuntime.renderVectorPlane(board, slide.data, {
          ...
          onObjectChange: function(kind, id, geometry2, commit) {
            updateVisualObject(slide.id, id, kind, geometry2, commit);
          }
        });
  ...
  // src/recipes/hierarchicalGallery.js
  function createHierarchicalGallery(api) {
    const global = window;
    ...
    function hierarchicalGallery(canvas, slide) {
      ...
      if (!slide.data.selectors.length && Object.values(slide.data.pageSets).every(function(pages) {
        return pages.length === 1;
      })) body.classList.add("static-gallery");
      var controls = document.createElement("div");
      ...
      slide.data.selectors.forEach(function(selector) {
        defaults[selector.id] = selector.options[0].value;
      });
      ...
      function componentText(componentId) {
        return effectiveComponent(slide, componentId).text;
      }
      function activeView() {
      ...
      function renderGallery() {
        ...
        if (slide.data.paired) {
          ...
          for (let column = 0; column < columns.length; column += 2) {
            ...
            group.className = "paired-gallery-group";
            var headings = document.createElement("div");
            headings.className = "paired-gallery-headings";
            ...
            page.rows.forEach(function(row) {
              ...
              pictures.className = "gallery-pair-pictures";
              ...
              captionFrame.className = "gallery-pair-caption-frame";
              if (captionId) {
              ...
              pictures.setAttribute("role", "button");
              pictures.tabIndex = 0;
              ...
              function enlarge(event) {
                ...
                dialog.className = "gallery-pair-dialog";
                var close = document.createElement("button");
                close.textContent = "Close \xD7";
                ...
                images.className = "gallery-pair-enlarged";
                ids.forEach(function(id, index) {
                  var figure = document.createElement("figure");
                  var label = document.createElement("figcaption");
                  label.textContent = componentText(columns[column + index]);
                  var img = document.createElement("img");
                  var source = effectiveComponent(slide, id);
                  img.src = source.src;
                  img.alt = source.alt;
        ...
        if (columns.every(function(key) {
          return !componentText(key).trim();
        })) grid.classList.add("without-column-labels");
        if (page.rows.every(function(row) {
          return !componentText(row.label).trim() && !row.detail;
        })) grid.classList.add("without-identities");
        ...
        columns.forEach(function(componentId) {
          var frame = document.createElement("div");
          frame.className = "gallery-heading-region";
        ...
        page.rows.forEach(function(row) {
          var frame = document.createElement("div");
          frame.className = "gallery-identity-region";
          ...
          row.images.forEach(function(componentId) {
            grid.appendChild(galleryImage(slide, componentId));
          });
        });
  ...
  function createScientificSlideRecipes(api) {
    const global = window;
    ...
    return {
      "evidence-figure": function(canvas, slide) {
        var body = document.createElement("div");
        body.className = "recipe-body evidence-figure-body";
        var labels = document.createElement("div");
        labels.className = "evidence-figure-labels";
        (slide.data.labels || []).forEach(function(key) {
          labels.appendChild(editableText(slide, key, "div", "evidence-figure-label"));
        });
        body.appendChild(labels);
        var frame = galleryImage(slide, slide.data.image);
        frame.classList.add("evidence-figure-frame");
      ...
      "hero-equation": function(canvas, slide) {
        var body = document.createElement("div");
        body.className = "recipe-body hero-equation-body";
        ...
        definitions.className = "hero-equation-definitions";
        slide.data.definitions.forEach(function(key) {
          definitions.appendChild(editableText(slide, key, "div", "hero-equation-definition"));
        });
      ...
      annotations,
      "chart-panels": function(canvas, slide) {
        return global.renderScientificChartPanels(canvas, slide, api);
      },
      "hero-plot": heroPlot,
